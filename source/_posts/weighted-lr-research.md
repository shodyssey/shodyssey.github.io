...
YouTube 深度学习推荐系统的 Ranking Model 输出层采用了 **Weighted Logistic Regression**，且在模型 Serving 时没有使用标准的 sigmoid 函数，而是使用了 $e^{Wx+b}$ 这一指数形式。这一设计引发了广泛讨论：为什么用 Weighted LR？为什么 Serving 时用指数形式而非 sigmoid？预测的到底是什么？
...
> **这一步是理解 YouTube Serving 函数的关键**：$e^{Wx+b}$ 计算的正是 Odds，而非概率 $p$。
...
**真相大白**：$T_i \cdot p$ 就是用户观看某视频的**期望时长**！
...
Serving 时只需关注**相对排序**（哪个视频期望观看时长更长），不关注绝对值，因此用 $e^{Wx+b}$ 作为排序指标完全合理。
...
Weighted LR 的 $Odds \approx E(T_i)$ 近似**仅在 pCTR 很小时成立**。当 pCTR 较大时（如 50%），近似严重失效。
...
Weighted LR 将时长预测巧妙转化为分类问题，兼顾了准确性和业务目标。
...
**模型收敛时，全局 odds 精确等于「加权正样本总量 ÷ 负样本数」**——这正是「平均每次曝光对应的期望观看时长」的经验版本。
...
> **结论**：低估时梯度大、高估时梯度小，模型倾向于**高估**观看时长。这是 WCE 在实际应用中需要注意的重要局限。
...
> 使用前需验证数据分布是否与几何分布匹配，否则考虑其他建模方式。
...
> 但此方法存在梯度不对称问题（见 6.4 节），低估时梯度大、高估时梯度小，需结合实际场景权衡。
...
**一句话总结**：YouTube 用观看时长作为正样本权重训练 LR，使得模型输出的 Odds 近似等于期望观看时长，Serving 时直接用 $e^{Wx+b}$（即 Odds）排序，巧妙地将时长预测转化为分类问题。
